...
export function MetaMensalForm({ metaAtual }: { metaAtual: number | null }) {
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-3 bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
      <div>
        <label className="block text-xs text-slate-500 mb-1">Meta de faturamento mensal (R$)</label>
        <input
          name="meta"
