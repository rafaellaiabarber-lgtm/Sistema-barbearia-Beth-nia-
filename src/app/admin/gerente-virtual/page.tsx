  ...
  return (
    <div>
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            ...
            <p className="text-xl font-bold text-blue-600">{formatarReais(comparativoAtual.faturamentoCentavos)}</p>
          </div>
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            <p className="text-slate-500 text-xs">Margem</p>
            <p className={`text-xl font-bold ${margemAtual < 0 ? "text-red-600" : "text-blue-600"}`}>
              {margemAtual.toFixed(0)}%
            </p>
          </div>
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            <p className="text-slate-500 text-xs">Ticket médio</p>
            <p className="text-xl font-bold text-blue-600">{formatarReais(ticketMedioAtual)}</p>
          </div>
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            <p className="text-slate-500 text-xs">Clientes novos</p>
            <p className="text-xl font-bold text-blue-600">{comparativoAtual.clientesNovos}</p>
          </div>
        </div>

        {alertas.length === 0 ? (
          <p className="text-slate-400 bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            ...
            {alertas.map((a, i) => (
              <div
                ...
                className={`rounded-xl p-4 shadow-sm border ${
                  a.tipo === "atencao" ? "bg-amber-50 border-amber-200 text-amber-800" : "bg-green-50 border-green-200 text-green-800"
                }`}
              >
                {a.tipo === "atencao" ? "⚠️ " : "✅ "}
                {a.texto}
      ...
      <section className="mb-8">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm mb-3">
          <p className="text-slate-500 text-sm mb-1">
            No ritmo atual ({formatarReais(previsao.ritmoDiarioCentavos)}/dia), você deve fechar o mês em:
          </p>
          <p className="text-2xl font-bold text-blue-600 mb-3">{formatarReais(previsao.previsaoFechamentoCentavos)}</p>
          ...
          {metaCentavos !== null ? (
            ...
            <p className="text-slate-400 text-sm">Defina uma meta mensal abaixo pra ver quanto falta por dia.</p>
          )}
        </div>
      ...
      <section>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          ...
          <form method="get" className="flex items-center gap-2">
            <label className="text-slate-500 text-sm">Sem voltar há mais de</label>
            <input
              type="number"
              name="diasSumidos"
