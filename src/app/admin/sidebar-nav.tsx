  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <div className="relative mb-4">
        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-lime-400" />
        <input
          value={busca}
          onChange={(e) => setBusca(e.target.value)}
          placeholder="Procurar opção do menu..."
          className="w-full rounded-lg bg-white/5 border border-lime-500/20 pl-9 pr-3 py-2 text-sm text-white placeholder:text-lime-400/60 focus:outline-none focus:ring-2 focus:ring-lime-400"
        />
      </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                  ativo ? "bg-lime-400 text-slate-950" : "text-lime-100/80 hover:bg-lime-500/10 hover:text-white"
                }`}
              >
            ...
            <div key={g.titulo}>
              <button
                ...
                className={`w-full flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
                  algumAtivo && colapsado ? "bg-lime-500/15 text-white" : "text-lime-100/80 hover:bg-lime-500/10 hover:text-white"
                }`}
              >
                <Icone className="w-4 h-4 shrink-0" />
              ...
              {!colapsado && (
                <div className="flex flex-col gap-1 mt-1 ml-4 pl-3 border-l border-lime-500/20">
                  {g.links.map((l) => {
                    ...
                    return (
                      <Link
                        ...
                        className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm transition-colors ${
                          ativo ? "bg-lime-400 text-slate-950 font-medium" : "text-lime-200/80 hover:bg-lime-500/10 hover:text-white"
                        }`}
                      >
