...
export function MetaBonificacaoForm({
  ...
  return (
    <form ref={formRef} action={formAction} className="flex flex-wrap items-end gap-3">
      <div>
        <label className="block text-xs text-slate-400 mb-1">Meta de faturamento/mês (R$)</label>
        <input
          name="meta"
          placeholder="sem meta"
          defaultValue={metaFaturamentoCentavos !== null ? (metaFaturamentoCentavos / 100).toFixed(2).replace(".", ",") : ""}
          className="w-32 rounded-lg bg-white border border-slate-300 px-2 py-1.5 text-sm"
        />
      </div>
      <div>
        <label className="block text-xs text-slate-400 mb-1">Bonificação ao bater a meta (R$)</label>
        <input
          name="bonificacao"
