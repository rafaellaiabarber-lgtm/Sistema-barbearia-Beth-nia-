...
export function MobileNav({ grupos, nome, logoUrl }: { grupos: NavGrupo[]; nome: string; logoUrl: string | null }) {
  ...
  return (
    <div className="md:hidden border-b border-orange-500/10 bg-neutral-950 relative">
      ...
      {aberto && (
        <nav className="absolute left-0 top-full z-10 w-64 max-h-[80vh] overflow-y-auto bg-neutral-950 border border-orange-500/10 rounded-br-xl shadow-xl flex flex-col gap-3 p-3">
          {grupos.map((g) => (
            <div key={g.titulo ?? "raiz"}>
              {g.titulo && (
                <p className="px-3 mb-1 text-xs font-semibold uppercase tracking-wide text-orange-400">{g.titulo}</p>
              )}
