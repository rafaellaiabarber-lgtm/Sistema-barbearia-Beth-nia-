  ...
  return (
    <div
      className={`bg-white border rounded-xl p-4 shadow-sm ${
        atrasada ? "border-red-300" : "border-slate-200"
      } ${conta.status === "PAGO" ? "opacity-60" : ""}`}
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="font-medium">{conta.descricao}</p>
          <p className={`text-xs mt-0.5 ${atrasada ? "text-red-600 font-semibold" : "text-slate-400"}`}>
        ...
        <div className="flex items-center gap-3">
          <span className="font-semibold text-slate-700">{formatarReais(conta.valorCentavos)}</span>
      ...
      {pagando && (
        <form action={formAction} className="mt-3 pt-3 border-t border-slate-100">
          <p className="text-slate-600 text-sm font-medium mb-2">Forma de pagamento:</p>
