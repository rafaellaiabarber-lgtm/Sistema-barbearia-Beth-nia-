      ...
      <div className="space-y-2 mb-2">
        {niveis.map((n, i) => (
          <div key={i} className="flex flex-wrap items-end gap-2">
            <div>
              <label className="block text-xs text-slate-500 mb-1">Nível {i + 1}</label>
              <input
                name="nivelNome"
                required
                placeholder="Nome"
                ...
                className="rounded-lg bg-white border border-slate-300 px-2 py-1.5 text-sm w-28"
              />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">{emCentavos ? "Alvo (R$)" : "Alvo (qtd)"}</label>
              <input
                name="nivelValor"
                required
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1">Bônus (R$)</label>
              <input
                name="nivelBonificacao"
                placeholder="0,00"
                value={n.bonificacao}
                onChange={(e) => atualizar(i, "bonificacao", e.target.value)}
                className="rounded-lg bg-white border border-slate-300 px-2 py-1.5 text-sm w-24"
              />
            </div>
            {niveis.length > 1 && (
              <button
                type="button"
                onClick={() => remover(i)}
                className="text-slate-400 hover:text-red-600 text-sm py-1.5"
              >
                Remover
              </button>
            )}
