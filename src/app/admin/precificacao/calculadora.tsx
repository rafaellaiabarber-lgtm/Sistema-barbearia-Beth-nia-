    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Taxa da maquininha (%)</label>
              <input
                value={taxaCartao}
            ...
            <div>
              <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Comissão do barbeiro (%)</label>
              <input
                value={comissao}
            ...
            <div>
              <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Imposto (%)</label>
              <input
                value={imposto}
            ...
            <div>
              <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Margem de lucro desejada (%)</label>
              <input
                value={margem}
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm">
        ...
        {percentualInvalido ? (
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm">Preencha o custo pra calcular.</p>
        ) : (
          <>
            ...
            <div className="space-y-1.5 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-slate-500 dark:text-slate-400">Custo do produto/insumo</span>
                <span>{formatarReais(reaisParaCentavos(custo))}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500 dark:text-slate-400">Taxa da maquininha</span>
                <span>{formatarReais(resultado.taxaCartaoCentavos)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500 dark:text-slate-400">Imposto</span>
                <span>{formatarReais(resultado.impostoCentavos)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500 dark:text-slate-400">Comissão do barbeiro</span>
