...
export default async function DrePage({
  ...
  return (
    <div>
      ...
      <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-sm overflow-hidden mb-6">
        {linhas.map((l) => (
          <div
            key={l.label}
            className={`flex items-center justify-between px-5 py-3 text-sm ${
              l.subtotal ? "bg-neutral-50 dark:bg-black font-semibold border-t border-b border-neutral-200 dark:border-neutral-800" : ""
            } ${l.destaque ? "bg-orange-50 dark:bg-orange-950 font-bold text-base" : ""} ${
              !l.subtotal && !l.destaque ? "border-b border-neutral-100 dark:border-neutral-800 last:border-b-0" : ""
            }`}
          >
            <span className={l.destaque ? "text-orange-900 dark:text-orange-200" : "text-neutral-700 dark:text-neutral-200"}>{l.label}</span>
            <span
              className={
                l.destaque
                  ? l.valorCentavos < 0
      ...
      <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-5 shadow-sm">
        <p className="text-neutral-500 dark:text-neutral-400 text-sm">Margem líquida no período</p>
        <p className={`text-2xl font-bold ${lucroLiquidoCentavos < 0 ? "text-red-600" : "text-orange-600 dark:text-orange-400"}`}>
          {margemPercentual}%
        </p>
      </div>

      <p className="text-neutral-400 dark:text-neutral-500 text-xs mt-6">
